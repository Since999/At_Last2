...
{
	CTexturePool::GetInstance()->SetDecriptorHeap(pd3dCommandList);
...
void CTexture::LoadTextureFromDDSFile(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, const wchar_t* pszFileName, UINT nResourceType, UINT nIndex)
{
	...
	m_ppd3dTextures[nIndex] = ::CreateTextureResource(pd3dDevice, pd3dCommandList, pszFileName, &m_ppd3dTextureUploadBuffers[nIndex], D3D12_RESOURCE_STATE_GENERIC_READ/*D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE*/);
}
...
void CMaterial::SetShadowShader(CShadowShader* shader)
{
	if (m_pShadowShader) m_pShadowShader->Release();
	m_pShadowShader = shader;
	if (m_pShadowShader) m_pShadowShader->AddRef();
}
...
void CMaterial::ReleaseShaderVariables()
{
	...
	if (m_pShadowShader) m_pShadowShader->ReleaseShaderVariables();
...
void CGameObject::SetNumberOfMeshes(int nmeshes)
{
	CMesh** tmp = NULL;
	if (nmeshes > 0)
	{
		tmp = new CMesh * [nmeshes];
		for (int i = 0; i < nmeshes; i++) tmp[i] = NULL;
	}
	int num = min(nmeshes, m_nMeshes);
	if (m_ppMeshes) {
		for (int i = 0; i < num; i++) tmp[i] = m_ppMeshes[i];
		delete[] m_ppMeshes;
	}
	m_nMeshes = nmeshes;
	m_ppMeshes = tmp;
...
void CGameObject::SetShadowShader(CShadowShader* shader)
{
	if (!m_pMaterial)
	{
		CMaterial* pMaterial = new CMaterial();
	...
	if (m_pMaterial) m_pMaterial->m_pShadowShader = shader;
}
...
void CGameObject::ShadowMapRender(ID3D12GraphicsCommandList* pd3dCommandList, CCamera* pCamera)
{
	if (m_pMaterial)
	{
...
void CGameObject::Scale(float size)
{
	XMMATRIX mtxScale = XMMatrixScaling(size, size, size);
...
void DebugObject::Render(ID3D12GraphicsCommandList* pd3dCommandList)
{
	...
	if (m_pMaterial)
	{
		if (m_pMaterial->m_pShader)
		{
			m_pMaterial->m_pShader->Render(pd3dCommandList, NULL);
			m_pMaterial->m_pShader->UpdateShaderVariables(pd3dCommandList);
		}
	}
